...
const main = async () => {
  ...
  allDocs.forEach(doc => {
    const data = doc.data();
    if (data.name === "St. Mary's Basilica" || data.name === "Centenary Methodist English Church") {
      oldChurch = data;
    }
  ...
  fs.writeFileSync('schema_inspection2.json', JSON.stringify({ oldChurch, newChurch }, null, 2));
  console.log('Inspection 2 complete.');
  process.exit(0);
};
